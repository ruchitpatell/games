...
class Tilemap:
    def __init__(self, game, tile_size=16):
        ...
        self.tilemap = {}               # square tiles on floor, dict for efficiency
        self.offgrid_tiles = []
    ...
    def physics_rects_around(self, pos):
        ...
        for tile in self.tiles_around(pos):
            if tile['type'] in PHYSICS_TILES:
                rectangles.append(pygame.Rect(tile['pos'][0] * self.tile_size, tile['pos'][1] * self.tile_size, self.tile_size, self.tile_size))
        return rectangles


    def render(self, surface, offset=(0, 0)):
        """
        Offset: camera opposite movement
        """

        for tile in self.offgrid_tiles:
            surface.blit(self.game.assets[tile['type']][tile['variant']], (tile['pos'][0] - offset[0], tile['pos'][1] - offset[1]))

        # only blit tiles in view
